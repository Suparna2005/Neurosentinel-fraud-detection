...
import { ScanSearch, AlertTriangle, ShieldCheck, DownloadCloud } from 'lucide-react';
...
const Scanner = () => {
  ...
  // 1. User -> loads sample transaction
  ...
  const scanTransaction = async () => {
    ...
    try {
      ...
      console.error(err);
      alert("Error scanning transaction.");
    } finally {
      setScanning(false);
    }
  };
  ...
  return (
    <div className="fade-in">
      ...
      <p className="subtitle">Execute realtime ML predictions on transaction payloads.</p>

      <div className="grid-2">
        <div className="glass-card">
          <h2 style={{fontSize: '1.2rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem'}}>
            <DownloadCloud size={20} /> Data Import
          </h2>
        ...
        <div className={`glass-card result-card ${result ? (result.prediction === 1 ? 'danger' : 'success') : ''}`}>
          ...
          <div style={{display: 'flex', justifyContent: 'center', margin: '2rem 0'}}>
            <button 
              ...
              disabled={!transaction || scanning}
            >
              {scanning ? 'Analyzing Tree Paths...' : 'Scan Transaction'}
            </button>
          </div>
          ...
          {result && (
            <div className="fade-in" style={{textAlign: 'center', marginTop: '2rem', padding: '1rem', borderTop: '1px solid rgba(255,255,255,0.1)'}}>
              ...
              <div className={`status-badge ${result.prediction === 1 ? 'danger' : 'success'}`}>
                ...
                {result.prediction === 1 ? 'HIGH RISK / FRAUDULENT' : 'LOW RISK / LEGITIMATE'}
              </div>
